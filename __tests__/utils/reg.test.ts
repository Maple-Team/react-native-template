import { REGEX_BANK_CLABE, REGEX_BANK_CARD } from '../../src/utils/reg'
  ...
  describe('bank card reg test case suits', () => {
    describe('bank card type', () => {
      it("18's number should be true", () => {
        expect(REGEX_BANK_CLABE.test(randomNum(18))).toBe(true)
      })
    ...
    describe('bank clabe type', () => {
      it("16's number should be true", () => {
        expect(REGEX_BANK_CARD.test(randomNum(16))).toBe(true)
      })
      ...
      it.concurrent.each([
        ...
        expect(REGEX_BANK_CARD.test(`${a}`)).toBe(expected)
      })
